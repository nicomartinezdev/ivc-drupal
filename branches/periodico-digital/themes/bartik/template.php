...
function bartik_pager_next($variables) {
    ...
    if ($pager_page_array[$element] < ($pager_total[$element] - 1)) {
		...
		if (empty($text) || !is_numeric($text)) {
			$output = str_replace("</a>", "<img src='/themes/bartik/images/flecha-login.png' title='Ir a la p&aacute;gina siguiente' /></a>", $output);	
		}
	}
	return $output;
}
...
function bartik_pager_previous($variables) {
    ...
    if ($pager_page_array[$element] > 0) {
        $page_new = pager_load_array($pager_page_array[$element] - $interval, $element, $pager_page_array);
      	$output = theme('pager_link', array('text' => (is_numeric($text) && intval($text) < 10 ? "0" . $text : $text), 'page_new' => $page_new, 'element' => $element, 'parameters' => $parameters));
      	if (empty($text) || !is_numeric($text)) {
			$output = str_replace("</a>", "<img src='/themes/bartik/images/flecha-volver.png' title='Ir a la p&aacute;gina anterior' /></a>", $output);	
      	}
    }
    return $output;
}